...
const WEDDING_PLAN = [
  {
    ...
    sections: [
      { name:"📍&ensp;场地",   items:["宴会场地", "日期", "定金"] },
      { name:"🎨&ensp;策划",   items:["宴会布置", "流程安排", "花艺"] },
      { name:"📷&ensp;摄影",   items:["跟拍摄影师", "摄像（optional）"] },
      { name:"👨‍👩‍👧‍👦&ensp;宾客", items:["Guest List", "座位安排"] },
      { name:"🍽️&ensp;餐饮",   items:["菜单确认", "酒水饮料"] },
    ],
  },
  {
    title: "💒&ensp;领证",
    color: "var(--jp)",
    sections: [
      { name:"📍&ensp;地点",     items:["Appointment", "确认所需材料（dkl 证明材料）"] },
      { name:"📸&ensp;证件照",   items:["成都拍红底证件照"] },
      { name:"📷&ensp;跟拍",     items:["预约摄影师"] },
    ],
  },
  {
    title: "👰&ensp;婚纱照",
...
(function(){
  ...
  WEDDING_PLAN.forEach(block => {
    ...
    sec.innerHTML =
      ...
      `<h2>${block.title}<span class="gp">${done} / ${items.length} done</span></h2>` +
      `<div class="gbar"><i style="width:${pct}%;background:${block.color || "var(--sy)"}"></i></div>` +
